...
class YAxis(
    ...
    override fun prepareLabels() {
        ...
        val x = when (axisSide) {
            ...
            else -> if (axisPosition == AxisPosition.INSIDE) {
                axisBounds.left + horizontalMargin
            } else {
                axisBounds.right - horizontalMargin
            }
        ...
        for (label in labelList) {
            ...
            val axisValue = axisBounds.bottom - totalHeight * ratio
            if (label.isNeedCalculateText()) {
                label.text = if (formatter == null) {
                    "$axisValue" // 兜底
                } else {
                    val value = formatter.dataRenderer.findValueByY(axisValue)
            ...
            label.x = x
            label.y = when {
                ratio <= 0f -> axisValue - descent // 顶部对齐
                ratio >= 1f -> axisValue - ascent // 底部对齐
                else -> axisValue - (ascent + descent) / 2f // 居中对齐
            }
        }
    }
